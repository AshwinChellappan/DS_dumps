...
create table Bank_Inventory_pricing (Product varchar(15),Quantity numeric,Price real,purchase_cost decimal,Estimated_sale_price float,Month numeric );
...
select avg(purchase_cost+Estimated_sale_price) from Bank_Inventory_pricing where Month=2;

# Question 2:
# 2) Print average of estimated_sale_price upto two decimals from bank_inventory_pricing table.

select truncate(avg(Estimated_sale_price),2) from Bank_Inventory_pricing;

# Question 3:
# 3) Print the Products which are appearing in bank_inventory_pricing more than once during the month : 1
select Product from Bank_Inventory_pricing group by Month having count(*)>1 ;
...
select Product from Bank_Inventory_pricing where  purchase_cost> ifnull(estimated_sale_price,0)   ;
# Question 5:
# 5) Print the sum of  purchase_cost of Bank_Inventory_pricing table with default value of 2000/-
# if there is no value given
select sum(purchase_cost) from Bank_Inventory_pricing where ifnull(purchase_cost,0) ;
# Question 6:

# 6) Print unique records of bank_inventory_pricing without displaying the month.

select * from Bank_Inventory_pricing;
select distinct Product, Quantity, Price, purchase_cost, Estimated_sale_price,Estimated_sale_price from Bank_Inventory_pricing; 
...
select avg(Purchase_cost) from Bank_Inventory_pricing where ifnull(purchase_cost,Estimated_sale_price);

# Question 8:
# 8) Print the count of unique Products used in  Bank_inventory_pricing
select Product,count(*) from Bank_Inventory_pricing group by Product ;
...
select * from Bank_Inventory_pricing;
select sum(purchase_cost) from Bank_Inventory_pricing where Month between 1 and 2; 

# Question 11:
# 11) Print Products with an average value of Purchase_cost per product only when exceeding average of 6000.
select Product,if(avg(purchase_cost)>6000,avg(purchase_cost),0 ) as Average from Bank_Inventory_pricing     group by Product;
select Product,avg(purchase_cost) as Average from Bank_Inventory_pricing     group by Product having avg(purchase_cost)>6000;  
# Question 12:
...
select max(Estimated_sale_price) from Bank_Inventory_pricing where Quantity>4 and purchase_cost is not null;

# Question 15:
# 15) Print products whose average of purchase_cost per product is less than 200
#  from the table Bank_inventory_pricing
select * from Bank_Inventory_pricing;
select Product from Bank_Inventory_pricing  group by Product having avg(purchase_cost)>200;
# Question 16:
# 16) Print each Product with its highest estimated_sale_price in bank_inventory_pricing
select max(Estimated_sale_price) from Bank_Inventory_pricing  group by Product; 
# Question 17:
...
select Product,avg(Estimated_sale_price)+(Estimated_sale_price*0.15) from Bank_Inventory_pricing   group by Product having avg(purchase_cost)> avg(Estimated_sale_price) ;
# Question 18:
...
select avg(purchase_cost) from Bank_Inventory_pricing  where Product='BusiCard' and purchase_cost is null; 
# Question 19:
...
select avg(coalesce(Estimated_sale_price,purchase_cost)) from Bank_Inventory_pricing group by Product;
# Question 20:
# 20) Print products and their avg price on condition that products appeared in at least three different months.
select Product,avg(price),count( distinct months) from Bank_Inventory_pricing group by Product having count(distinct months)>2;