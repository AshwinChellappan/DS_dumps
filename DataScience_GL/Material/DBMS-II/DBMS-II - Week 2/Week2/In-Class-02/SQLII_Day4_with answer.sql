...
#Solution:
ALTER VIEW SLR_FOCUS
   AS SELECT `SERIAL NO.`, SOP, LOR, RESEARCH, (SOP + LOR) / 2 AS New_Score 
   FROM admission_predict
   WHERE Research = 1;
...
#Solution:
#Deadlock occurs in these situations in session -2 if SESSION -2 is not given its SELECT query with share lock. So, follow below.
